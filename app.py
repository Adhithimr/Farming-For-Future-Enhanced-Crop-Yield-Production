...
cr = pickle.load(open('models/RandomForest.pkl', 'rb')) # recommeend
...
def Yield():
    if request.method == 'POST':
        ...
        res = forest.predict([[float(state),
                                 float(district),
                                 float(year),
                                 float(season),
                                 float(crop),
                                 ...
                                 float(soilmoisture),
                                 float(area)]])[0]
        return render_template('yield.html', prediction=f"{res:.2f} Kgs")
    import requests
    import pandas as pd
    data=requests.get("https://api.thingspeak.com/channels/2355804/feeds.json?api_key=R1DOD6XV9AX6W0SF&results=2")
    p=data.json()['feeds'][-1]['field6']
    n=data.json()['feeds'][-1]['field4']
    k=data.json()['feeds'][-1]['field5']
    temp=data.json()['feeds'][-1]['field3']
    hum=data.json()['feeds'][-1]['field1']
    soil=data.json()['feeds'][-1]['field2']
    return render_template('yield.html', temp=temp, hum = hum, soil=soil)
...
def crop_price():
    if request.method == 'POST':
        ...
        out_1 = cp.predict([[float(state),
                                 float(district),
                                 float(year),
                                 float(season),
                                 float(crop)]])
        return render_template('crop_price.html', prediction=f"{out_1[0]:.2f}")
...
##            return render_template('crop.html', msg="Some thing went wrong, try again")
    ...
    hum=data.json()['feeds'][-1]['field1']
    soil=data.json()['feeds'][-1]['field2']
    return render_template('crop.html', p=p, n=n, k=k, temp=temp, hum=hum, soil=soil)
...
def market():
    ...
    if table:
        result = [['Commodity', 'Place', 'Variety',	'Average Price', 'Minimum Price', 'Last Updated On']]
